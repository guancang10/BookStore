...
func (b BookRepositoryImpl) Save(ctx context.Context, tx *sql.Tx, book domain.Book) domain.Book {
	script := "INSERT INTO Book(BookName,BookDescription,Author,CategoryId,Price,Qty,AuditUsername) VALUES(?,?,?,?,?,?,?)"
	result, err := tx.ExecContext(ctx, script, book.BookName, book.BookDescription, book.Author, book.CategoryId, book.Price, book.Qty, book.AuditUsername)
...
func (b BookRepositoryImpl) Get(ctx context.Context, tx *sql.Tx, bookId int) (domain.Book, error) {
	script := "SELECT Id,BookName,BookDescription,Author,CategoryId,Price,Qty FROM Book WHERE Id = ?"
	rows, err := tx.QueryContext(ctx, script, bookId)
	helper.CheckError(err)
	defer rows.Close()
	var result domain.Book
	if rows.Next() {
		err := rows.Scan(&result.Id, &result.BookName, &result.BookDescription, &result.Author, &result.CategoryId, &result.Price, &result.Qty)
		helper.CheckError(err)
		return result, nil
	} else {
...
func (b BookRepositoryImpl) GetAll(ctx context.Context, tx *sql.Tx) []domain.Book {
	script := "SELECT Id,BookName,BookDescription,Author,CategoryId,Price,Qty From Book"
	...
	for rows.Next() {
		...
		rows.Scan(&data.Id, &data.BookName, &data.BookDescription, &data.Author, &data.CategoryId, &data.Price, &data.Qty)
		helper.CheckError(err)
